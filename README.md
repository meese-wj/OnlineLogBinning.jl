...
Julia package to determine effective number of uncorrelated data points in a correlated data stream via an $\mathcal{O}\left(\log_2 N\right)$ online binning algorithm.

This package uses the online logarithmic binning algorithm discussed in Refs. [[1]](@ref) and [[3]](@ref), but uses the numerically stable _first-pass pairwise_ algorithm from Ref. [[2]](@ref) to update the mean and variance accumulators. Importantly, the binning analysis is _online_ in the sense that the whole data stream of size $N$ need not be stored; instead only $\mathcal{O}\left(\log_2 N\right)$ numbers need to be.
